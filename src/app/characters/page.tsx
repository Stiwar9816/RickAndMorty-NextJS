...
export default function CharacterPage() {
	...
	if (error) return <p className="font-mono text-base">Error: {error}</p>;
	...
	return (
		<>
			...
			<div className="flex max-w-full gap-6 my-2">
				<SearchInput />
				<FilterDropdown pageCategory={PAGE.CHARACTERS} />
			</div>
			<div className="flex flex-grow items-center justify-center">
				{isFilterEmpty ? (
					<div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-5 p-5">
						<CardCharacters characters={data} />
					</div>
				) : dataFiltered.length === 0 ? (
					<NotResults />
				) : (
					<div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-5 p-5">
						<CardCharacters characters={dataFiltered} />
					</div>
				)}
			</div>
			<Pagination />
